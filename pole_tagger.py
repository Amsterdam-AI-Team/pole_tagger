...
def click_event(event, x, y, flags, params):
	...
	if event == cv2.EVENT_LBUTTONDOWN:
		...
		if len(refPt) == 2:
			cv2.line(img_single_axis, (refPt[0][0], refPt[0][1]), (refPt[1][0], refPt[1][1]), (0, 0, 255), 2)
...
def adjust_pole_statistics(in_folder, idx, obj):
    in_file_ax_x = str(list(pathlib.Path(in_folder + 'object_per_axis/x/').glob('{}_*'.format(idx)))[0])
    in_file_ax_y = str(list(pathlib.Path(in_folder + 'object_per_axis/y/').glob('{}_*'.format(idx)))[0])
...
def check_poles(in_folder, in_folder_imgs, csv_poles, out_file):
    ...
    while idx < len(df_poles):
        ...
        if df_poles_adjusted.loc[idx, 'code'] < 0 or np.isnan(df_poles_adjusted.loc[idx, 'code']) == True:
