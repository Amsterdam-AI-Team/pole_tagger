        ...
        if df_poles_adjusted.loc[idx, 'code'] == -1:
            ...
            if os.path.exists(img_name):
                ...
                while k not in pos_values:
                    k = cv2.waitKey(0)
                    if k in [13, 32, 3, 81, 49]:  # [enter] or [space] or [>] or [1]: true positive
                        obj['code'] = 0
                        ...
                        obj['code'] = 3
                        continue
                    elif k in [8, 127, 2, 81] and idx > 0:  # [backspace] or [<]: go back one example
...
def validate_type(in_folder, in_folder_imgs, out_file):
    ...
    sub_df = df_poles_adjusted.loc[(df_poles_adjusted['type'].isna()) 
                                   & (df_poles_adjusted['code'].isin([0, 4]))]
...
if __name__ == "__main__":
    ...
    if args.validate_pole:
        ...
        try:
            ...
            print('Make sure that (some) poles are already validated using the --validate_pole argument.')
            sys.exit(1)
        ...
        try:
            ...
            print('Make sure that (some) poles are already validated using the --validate_poles argument.')
            sys.exit(1)